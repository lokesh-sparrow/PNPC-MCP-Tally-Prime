...
| `create_delivery_note` | `date`, `narration?`, `partyLedger`, `items` (`stockItem`, `qty`, `rate`, `unit`, `salesLedger`, `godown?`, `batchName?`, `discountPercent?`), `voucherNumber?` | Creates a Delivery Note (`VCHTYPE="Delivery Note"`, `OBJVIEW="Invoice Voucher View"`, `ISINVOICE=No`) — item-line dispatch of goods before/without a full Sales invoice. **Confirmed live:** the voucher type must be active in the company (a company-level toggle, same as `set_bill_of_materials`'s prerequisite) or the API reports `CREATED:1` for a voucher that shows in no report at all. **Also confirmed live:** even with the type active, `get_vouchers`/`get_ledger_vouchers`/`update_voucher`/`delete_voucher` could not find a Delivery Note created this way — manage it in Tally's own UI, not this connector, until that gap is resolved. Requires `godown` if the company has multi-godown tracking (same silent-fail behavior as `create_sales_invoice`). |
| `create_receipt_note` | `date`, `narration?`, `partyLedger`, `items` (`stockItem`, `qty`, `rate`, `unit`, `purchaseLedger`, `godown?`, `batchName?`, `discountPercent?`), `voucherNumber?` | Creates a Receipt Note (`VCHTYPE="Receipt Note"`) — the buying-side mirror of `create_delivery_note`, same two confirmed caveats (voucher type must be active; not findable afterward via `get_vouchers`/`get_ledger_vouchers`/`update_voucher`/`delete_voucher`). |
