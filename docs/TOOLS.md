...
| Tool | Args | Returns |
|---|---|---|
| `get_ledgers` | `query?` | All ledgers (name, parent group, closing balance, VAT TRN, VAT dealer type, state, country, GSTIN, PAN, GST registration type, GST type of supply, contact person), or — with `query` — a fuzzy-ranked shortlist of the closest-matching names (exact/prefix/substring, then a loose in-order character match for typos/abbreviations), capped to the top 20, best match first. GSTIN/PAN/GST registration type/GST type of supply are India-specific — a UAE company will have these blank and `trn`/`vatDealerType`/`state`/`country` populated instead, and vice versa. |
| `get_stock_items` | — | All stock items: name, parent group, closing balance, description, part number, costing method, GST type of supply, GST applicable (read-only — Tally computes this from the item's own GST configuration, it is never settable), plus `GSTDETAILS: {hsnCode, taxability, applicableFrom, isReverseChargeApplicable, gstIneligibleItc}` (or `null` if never set) — pulled from Tally's own date-versioned "GST Details" list (`$HSNCode`/`$Taxability`/`$IsReverseChargeApplicable`/`$GSTIneligibleITC`/`$ApplicableFrom` read from inside a `GSTDetails` `EXPLODE`, confirmed live — a flat Stock Item accessor for these always returns empty even when the item genuinely has them set). When an item has more than one dated GST Details entry, only the one with the latest `applicableFrom` is shown, not the full history. |
...
| `create_stock_item` | `name`, `group`, `unit`, `openingBalance?`, `openingRate?`, `description?`, `partNumber?`, `costingMethod?`, `hsnCode?`, `taxability?`, `gstDetailsApplicableFrom?`, `gstTypeOfSupply?`, `isReverseChargeApplicable?`, `gstIneligibleItc?`, `rateOfVat?`, `ignoreNegativeStock?`, `extraFields?` | Creates a new stock item. Same `"Primary"` handling as `create_group`. `costingMethod` is plain text matching Tally's own dropdown options exactly (e.g. `'Avg. Cost'`). `hsnCode`/`taxability`/`isReverseChargeApplicable`/`gstIneligibleItc` all write to the same entry in Tally's date-versioned GST Details list — same `GSTDETAILS.LIST`/`APPLICABLEFROM` pattern as `create_ledger`'s address, confirmed live (writing `<GSTDETAILS.LIST><APPLICABLEFROM>...<HSNCODE>...<TAXABILITY>...<ISREVERSECHARGEAPPLICABLE>...<GSTINELIGIBLEITC>...` and reading it back through the same nested list all round-tripped correctly); `gstDetailsApplicableFrom` defaults to today if any of those fields is set and it's omitted. `gstTypeOfSupply` ('Goods'/'Services') is a separate, flat item field, also confirmed live. `extraFields` is the same escape hatch as on `create_ledger`. |
...
| `create_voucher` | `voucherType`, `date`, `voucherNumber?`, `reference?`, `narration?`, `debitLedger`/`creditLedger`/`amount` (2-leg) or `entries?` (N legs), `debitBillName?`, `debitBillType?`, `creditBillName?`, `creditBillType?` | Creates a voucher (Payment, Receipt, Journal, Contra, ...) with 2 or more ledger lines. Pass `entries: [{ledgerName, amount, type: "debit"|"credit", billName?, billType?, costCentre?, costCategory?}]` for 3+ legs (e.g. one payment split across several expense ledgers) — validated client-side to balance before it ever reaches Tally. `debitBillType`/`creditBillType` is `"New Ref"` (open a new bill) or `"Agst Ref"` (settle an existing one by exact bill reference name) — only takes effect if the ledger has `maintainBillWise` enabled. `voucherNumber` overrides Tally's automatic numbering (normally omit it; pass it explicitly if a company's numbering series doesn't pick up correctly — confirmed live, a 'PNPC/2026/...' series silently restarted from '1'). `reference` maps to Tally's own voucher-level `<REFERENCE>` field (e.g. a supplier's bill number like `PI-061538`) — independent of `voucherNumber` and of the per-leg bill reference name/type. |
...
| `update_voucher` | `voucherType`, `voucherNumber`, `date`, `reference?`, `narration?`, `debitLedger`/`creditLedger`/`amount` **or** `entries?` (3+ legs, same shape as `create_voucher`) | Replaces the ledger entries/narration/`<REFERENCE>` of an existing voucher. **Matched by type + date + voucher number** — that combination must uniquely identify the voucher (confirm with `get_ledger_vouchers` first). |
...
| Tool | Args | Effect |
|---|---|---|
| `set_company` | `companyName` | Switches Tally's active company — persistent, global state, affects every subsequent call. Only works if that company is already open in Tally; verifies the switch actually happened and throws an error otherwise (Tally's own `ChangeCurrentCompany` action silently no-ops on a company that isn't open — this tool does not). **Also empties every SQL-cache table and the cached godown list** — both are per-company data with no company column of their own, so a stale switch would otherwise keep serving the previous company's rows/godowns under the new company's name (confirmed live: this was a real gap before the fix). |
...
| `sync_voucher_ledger_entries_to_sql` | `from`, `to` | Loads voucher ledger lines (ledger name, amount, cost centre, bill allocation) for one date range into the same cache — the piece `get_vouchers`/`sync_vouchers_to_sql` can't give you, since those only carry each voucher's overall total, not which ledgers it posted to. Reuses the same TDL template `verifyVoucherWrite` already relies on internally for post-write verification, now exposed for bulk historical queries. `amount` is signed (negative debit, positive credit, confirmed live against a real Sales invoice whose lines summed to zero) — the intended use is splitting a combined ledger's balance apart by voucher (e.g. a VAT ledger into Output vs Input via `GROUP BY ledger, voucher_type`), which the connector otherwise has no way to answer. |
