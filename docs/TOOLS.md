...
| `get_vat_return_box_summary` | `from`, `to` | UAE VAT 201 return boxes for a period, built from the same ledger rows as `get_vat_liability_summary` — not a new query, that data regrouped. Returns `box1` (standard-rated supplies output VAT, combined across emirates), `box3` (reverse charge output VAT), `box9` (standard-rated expenses input VAT), `box10` (reverse charge input VAT), `box11` (net VAT due), plus `sourceRows` (the ledgers behind each box) and `boxesNotCovered` — an explicit list of real Form 201 boxes (emirate-wise split of box1, tourist refunds, zero-rated supplies, exempt supplies, goods imported) that ledger closing balances genuinely can't answer, since they need voucher-level place-of-supply or rate data a ledger balance doesn't carry. Meant to sanity-check a return before filing, not replace Tally's own VAT Return report. |
