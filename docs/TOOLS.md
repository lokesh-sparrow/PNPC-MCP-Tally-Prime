...
| `update_delivery_note` | Same fields as `create_delivery_note`, plus required `voucherNumber` | Replaces an existing Delivery Note's item lines in place. Same voucher-type-active caveat. Matched by date + voucher number; refuses on a cross-type collision, same as every other `update_*` tool. |
